...
ansible_function() {
    ...
    -e TEMP_DESTINATION_BASE_PATH="${TEMP_DESTINATION_BASE_PATH}" \
    -e LOCAL_DESTINATION_BASE_PATH="${LOCAL_DESTINATION_BASE_PATH}" \
    -e ENV_NAME="${ENV}" \
    -e DEPLOYMENTS_FOLDER="${DEPLOYMENTS_FOLDER}" \
...
ansible_delete_backups_from_backup_machine_function() {
    ansible-playbook "${ANSIBLE_CLEAN_BACKUP_MACHINE_PLAYBOOK}" \
    -i "${BACKUP_MACHINE}," \
    -e ansible_ssh_pass="${ANSIBLE_SSH_PASS}" \
    -e ansible_become_pass="${ANSIBLE_BECOME_PASS}" \
    -e ANSIBLE_REMOTE_USER="${ANSIBLE_REMOTE_USER}" \
